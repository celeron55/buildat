...
**The focus is `builtin/luanti` until further notice.** Everything else in
this file that is unfinished is frozen: `undermine` (section 5), `aggregate`
(section 6) and `extensions/luanti_client` (section 9). All three are kept
compiling and working as the engine changes, and nothing more. `luanti_client`
in particular is parked rather than abandoned -- it comes back when
`builtin/luanti` reaches feature parity and the client half is what needs
doing, because at that point the module's forked client and this one are the
same problem and should be looked at together.
...
built; sections 12, 13 and 10 say what each one turned out to be.
