...
3. **Then v7 and the rest. Built 2026-09-13** for the whole tree: every
   one of Luanti's eight mapgens is compiled in and a world picks one with
   `mg_name`. What is left is what a game registers -- the biomes, the
   ores and the decorations -- which is stage 3c below. Two things stage 2 measured
...
### Mapgen stage 3c: the world a game registers (open)

Stages 3a and 3b are built: the whole of Luanti's `src/mapgen` is vendored
into `builtin/luanti_mapgen` on top of a shim, all eight mapgens are
compiled in, and a world whose `mg_name` says `v7` is generated by v7 in
`worldgen`'s worker thread. A new world's mapgen is v7 and is written into
the save the first time it is opened, which is what Luanti keeps in
map_meta.txt; a save that was played when singlenode was all there was
keeps singlenode. What that turned out to be is in
`doc/plan/luanti_module_history.md`, "The mapgen, vendored".

Three of the shim's own numbers turned out to matter, and all three are
settled: the reserved content ids in `vendor/mapnode.h` are the module's
(ignore 0, unknown 1, air 2) and not Luanti's, because a mapgen writes air
by id and the module's registry hands out its own -- `VendoredGenerator`
checks them against what the module sent; the node definition shim answers
"found" for a name it knows rather than "not ignore", since ignore is a
node with an id like any other here; and the `EmergeParams` a mapgen is
built with belongs to the mapgen, which deletes it, along with the managers
and the biome generator it owns.

**The biomes are built (2026-09-13), and the node properties with them.**
`core.__mapgen_biomes()` hands over each registered biome with its node
names already the ids they mean, and `core.__mapgen_node_props()` hands over
what a mapgen asks about a node -- `is_ground_content`, `floodable`, the
liquid type, what light does -- so the shim stops guessing. devtest's world
is sand under the water line and dirt with grass above it. What it took
besides the crossing: a section's own terrain has to win over the padding a
neighbour's generator wrote into it, which is terrain from before that
mapgen's biomes ran, so `voxelworld::merge_volume()` takes the region the
writer owns.
...
world generates the default biome out of `mapgen_stone` and nothing else.
`core.registered_biomes`, `_ores` and `_decorations` have been recorded on
the Lua side since M2 -- the translation between the two is the work, and it
is what makes a game's world look like that game's.

The order inside it is the managers' own dependency order: biomes, then
ores, then decorations, which want schematics and the tree generator.

**And two things beside it:**

- **What a mapgen asks about a node is answered by a guess.** The
  `ContentFeatures` the shim builds says that everything which is not air
  is solid ground. The game's own answers -- `is_ground_content`,
  `liquid_type`, `floodable`, `walkable` -- are not sent across yet; a cave
  carved through a chest is the sign of it. Small, and it goes with 3c
  because the same packet carries it.
- **A schematic file is not read.** `vendor/serialization.cpp` says why:
  the compression and `MapNode::serializeBulk`. A schematic a mod defines
  in a Lua table does not go through either, so this only matters for a
  game that ships `.mts` files -- which most do.

**The decisions that govern it, taken 2026-09-13:**

- **Where it lives:** `builtin/luanti_mapgen`, a module of its own, because
  a runtime-compiled module is a single translation unit and
  `builtin/luanti` is the file that changes most. It compiles in six
  seconds and only when its own files change.
- **How faithful:** a world that *looks like* a Luanti world rather than
  one that reproduces upstream's terrain for a seed, so the noise is
  buildat's own vendored copy and the shim adapts Luanti's calls onto it.
- **The light is the mapgen's**, and what keeps it right afterwards is
  settled: see "Skylight over a streamed world" below.
- **Settings answer nothing**, so a mapgen's own C++ defaults stand, with
