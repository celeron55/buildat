...
1. **3a: the seam itself. Built 2026-09-13, half of it.** The module
   ...
   **3a and 3b are built (2026-09-13).** The whole tree compiles, and a
   world whose `mg_name` says `v7` is generated by Luanti's v7: the module
   builds a `NodeDefManager` out of the content ids and aliases
   `builtin/luanti` sends over, a `MapgenParams` with the world's seed and
   a section for a chunk, the four managers and a biome generator, and
   `Mapgen::createMapgen()`; worldgen's thread calls `makeChunk()` per
   section and copies the result into the volume. All eight of Luanti's
   mapgens are compiled in and `list_mapgens()` names them.
   ...
   - **What a mapgen asks about a node is answered by a guess.** The
     ...
     `liquid_type`, `floodable` -- are not sent across yet, and a cave
     carving through a chest would be the sign of it.
   - **Biomes, ores and decorations a mod registers do not reach the
     managers.** `core.registered_biomes`, `_ores` and `_decorations` are
     recorded in Lua and nothing translates them into `BiomeManager` and
     its friends yet, so what generates is the default biome only. This is
     stage 3c and it is the biggest of what is left.
   - **A schematic file is not read** (`vendor/serialization.cpp` says
     why), and `Schematic::placeOnMap()` has a `ServerMap` that does
     nothing.
