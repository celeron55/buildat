...
makes it yellow-green instead is light coming through the leaf, which is a
transmission term rather than anything to do with reflections: tinting the
cube map towards the sun would not reach it, because in that geometry the
surface reflects the sky behind the camera, away from the sun.
...
keys 1, 2 and 3:
    ...
    5 Against sun   downwind of the sun from a canopy, above its top face

3 is the one that matters most for skylight: dark rock around a blown-out
opening. 4 and 5 are the ones for materials. A reflection is strongest at a
grazing angle, so 4 says whether the cube map is reaching the water, with
leaves and grass in the same frame for contrast. 5 puts the sun behind and
above a tree, which is where transmission through the leaves either shows or
does not; the camera is high enough to see the canopy's top face as well as
the backlit side, so the directly lit and the transmitted sides are in the
same frame.
...
check.txt visits all five benchmarks and screenshots each, for comparing a
...
It also shoots both benchmark edits, so a run is: all five views of the scene
...
camera goes in main:water, and which tree benchmark 5 looks at in main:tree, so
...
terrain height around the pond to stay above ground; benchmark 5's is worked
out on the client because it needs the sun direction, which lives there. VIEW_DIR is the one thing still
