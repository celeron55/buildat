...
namespace c55
{
	...
	inline std::string vector_join(const std::vector<std::string> &v, const std::string &d=", ")
	{
		std::string result;
		for(auto v1 : v)
	...
	inline std::string truncate_string(const std::string &str, size_t maxlen, const std::string &end="...")
	{
		if(str.size() < maxlen)
			return str;
